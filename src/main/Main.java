	...
	public static void main(String[] args) {		
		...
		for (i=0 ; i<50 ; i++) {
			...
			while (ePop.hasMoreElements()) {
				...
				f.getPopulation().get(key).Comportement(f);
				
			}
			System.out.println("------Fin du cycle------");
			System.out.println("***Messages***");			
				
			Enumeration<MessageAbstract> eMessages = Collections.enumeration(BoiteMessagesSingleton.getInstance().getMessagesArrayList());
			
			while (eMessages.hasMoreElements()) {
				
				MessageAbstract m = eMessages.nextElement();
